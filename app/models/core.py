...
class Answer(db.Model, BaseMixin, DateTimeMixin):
    ...
    favorites = db.relationship('AnswerFavorite', lazy='dynamic', backref='answer',
                                foreign_keys=[AnswerFavorite.answer_id], cascade='all,delete-orphan')
...
class Question(db.Model, BaseMixin, DateTimeMixin):
    ...
    topics = db.relationship('QuestionTopic', backref='question', lazy='dynamic',
                             foreign_keys=[QuestionTopic.question_id])  # 属于的话题
    favorites = db.relationship('QuestionFavorite', backref='question', lazy='dynamic',
                                foreign_keys=[QuestionFavorite.question_id], cascade='all,delete-orphan')
...
class Topic(db.Model, BaseMixin, DateTimeMixin):
    ...
    followers = db.relationship('FollowTopic', backref=db.backref('followed', lazy='joined'),
                                ...
                                cascade='all,delete-orphan')
    questions = db.relationship('QuestionTopic', backref='topic', lazy='dynamic',
...
class Favorite(db.Model, BaseMixin, DateTimeMixin):
    ...
    questions = db.relationship('QuestionFavorite', backref='favorite', lazy='dynamic',
                                foreign_keys=[QuestionFavorite.favorite_id], cascade='all,delete-orphan')
    answers = db.relationship('AnswerFavorite', backref='favorite', lazy='dynamic',
                              foreign_keys=[AnswerFavorite.favorite_id], cascade='all,delete-orphan')
...
class User(db.Model, UserMixin, BaseMixin):
    ...
    private_messages = db.relationship('Message', backref='sender', lazy='dynamic',
                                       foreign_keys=[Message.sender_id])  # 我发送的私信
    private_messages_from = db.relationship('Message', backref='receiver', lazy='dynamic',
                                            foreign_keys=[Message.receiver_id])  # 我接收的私信
